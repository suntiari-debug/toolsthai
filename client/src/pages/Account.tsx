...
type ProfileForm = { name: string; address: string; taxId: string; phone: string; email: string; logoDataUrl: string; existingLogoUrl: string; signatureDataUrl: string; existingSignatureUrl: string; stampDataUrl: string; existingStampUrl: string };
const emptyProfile: ProfileForm = { name: "", address: "", taxId: "", phone: "", email: "", logoDataUrl: "", existingLogoUrl: "", signatureDataUrl: "", existingSignatureUrl: "", stampDataUrl: "", existingStampUrl: "" };
...
function AssetUpload({ label, description, previewUrl, onChange }: { label: string; description: string; previewUrl: string; onChange: (event: ChangeEvent<HTMLInputElement>) => void }) { return <div className="account-document-asset"><div className="account-document-asset-preview">{previewUrl ? <img src={previewUrl} alt={`ตัวอย่าง${label}`} /> : <WandSparkles size={17} />}</div><div><strong>{label}</strong><small>{description}</small></div><label className="upload-label"><Upload size={14} /> อัปโหลด<input type="file" accept="image/png,image/jpeg,image/webp" onChange={onChange} /></label></div>; }
