...
import { CANONICAL_ORIGIN, SITE_NAME, type SsrHead } from "../../shared/seo";
...
function composeHtml(template: string, html: string, head: SsrHead, state: unknown) {
  ...
  const canonical = `${CANONICAL_ORIGIN}${head.canonicalPath}`;
...
export async function setupVite(app: Express, server: Server) {
  ...
  app.use(async (req, res, next) => {
    ...
    try {
      ...
      res.status(200).set("Cache-Control", "no-cache").type("html").end(composeHtml(template, result.html, result.head, result.dehydratedState));
